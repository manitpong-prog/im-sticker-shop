...
const CATEGORY = 'top_creator_id';
const BASE_URL = 'https://store.line.me/stickershop/showcase/top_creators/id';

const START_PAGE = 10;
const END_PAGE = 1;

const TARGET_PRICE = 2400;
const PAGE_DELAY_MS = 2000;
const PRODUCT_DELAY_MS = 2000;
// ==============

async function runScraperIndo10cAnimated() {
  ...
  console.log(
    '\n==================== 🇮🇩 เริ่มเก็บ สติกเกอร์ดุ๊กดิ๊ก 10c อินโด ===================='
  );
  ...
  try {
    for (let pageNum = START_PAGE; pageNum >= END_PAGE; pageNum--) {
      ...
      try {
        ...
        console.log(`   📦 พบทั้งหมด ${productLinks.length} รายการในหน้า ${pageNum}`);
        ...
        for (let i = 0; i < productLinks.length; i++) {
          ...
          try {
            ...
            await page
              .waitForSelector('.mdCMN38Item01Price, .mdCMN38Price, .mdCMN08Price', {
                timeout: 30000,
              })
              .catch(() => null);
            ...
            try {
              ...
              console.log(`      ⏩ ข้าม ID: ${pId} เพราะอ่าน data-preview ไม่ได้`);
              continue;
            }
            ...
            if (upsertError) {
              ...
              console.log(`      ✅ บันทึกสำเร็จ: "${title}"`);
            }
  ...
  console.log('\n🏁 เก็บสติกเกอร์ดุ๊กดิ๊ก 10c อินโดเสร็จสิ้น!');
}

runScraperIndo10cAnimated();